document.addEventListener('DOMContentLoaded', () => {
        ...
        inputs.forEach(input => {
            ...
            if(window.activeElement) {
                ...
                if(cls.includes('Button')) {
                     ...
                     if(type === 'hover_color') input.value = a.getAttribute('data-hover-color') || '#ffffff';
                     if(type === 'background-color') input.value = a.getAttribute('data-original-bg') || a.style.backgroundColor;
                     if(type === 'color') input.value = a.getAttribute('data-original-color') || a.style.color;
                }
            }
            ...
            input.addEventListener('input', function() {
                ...
                if (['layout', 'shape', 'icon_type', 'hover_style', 'text-transform'].includes(type)) {
                ...
                switch (type) {
                    ...
                    case 'shape':
                        if(cls.includes('Socials')) {
                            ...
                            links.forEach(a => {
                                a.className = 'social-link hover:opacity-80 transition flex items-center justify-center'; 
    ...
    function getStyleTarget(root, cls) {
        if(!cls) return root; // Case for builder-row
    ...
    function renderElementPreview(name, className, contentData = {}) {
        ...
        if (className.includes('Button')) {
            ...
            if (c.layout === 'icon') content = `<div class="search-icon-only cursor-pointer hover:opacity-70" style="color: inherit; font-size: 20px;"><i class="ph ph-magnifying-glass"></i></div>`;
            else {
