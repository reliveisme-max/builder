document.addEventListener('DOMContentLoaded', () => {
    ...
    // --- 0. RESTORE DATA ---
    if (window.savedData && Array.isArray(window.savedData)) restoreLayout(window.savedData);
            ...
            for (const [zoneName, elements] of Object.entries(rowData.columns)) {
                ...
                elements.forEach(elData => {
                    ...
                    if (elData.content) {
                        ...
                        if(c.text) { const t=newEl.querySelector('.text-content, button, a, .inner-text'); if(t) t.innerText=c.text; const i=newEl.querySelector('input'); if(i) i.placeholder=c.text; }
                    ...
                    if (elData.style) {
                        let target = newEl;
                        if (elData.class.includes('Button') || elData.class.includes('CategoryBtn')) target = newEl.querySelector('a, button, .inner-box');
                        else if (elData.class.includes('Search')) target = newEl.querySelector('.search-box, .search-icon-only');
                        else if (elData.class.includes('Menu') || elData.class.includes('Socials')) target = newEl.querySelector('nav, .social-group');
                        else if (elData.class.includes('Divider')) target = newEl.querySelector('div'); 
                        
                        if(target) target.style.cssText = elData.style;
                        if(elData.class.includes('Search')) { const inp = target.querySelector('input'); if(inp) inp.style.backgroundColor='transparent'; }
    ...
    window.initLiveEdit = function() {
        ...
        if (cls.includes('Menu') || cls.includes('Socials')) {
            ...
            if (container && hiddenInput) {
                ...
                function renderRepeaterUI() {
                    ...
                    currentData.forEach((item, index) => {
                        ...
                        if (cls.includes('Menu')) {
                            row.innerHTML = `<div class="flex gap-2"><input type="text" value="${item.text}" data-idx="${index}" data-key="text" placeholder="Tên" class="rep-field flex-1 bg-gray-900 text-white text-xs p-1.5 rounded border border-gray-600"><button type="button" onclick="rmRepItem(${index})" class="text-gray-500 hover:text-red-500"><i class="ph ph-trash"></i></button></div><input type="text" value="${item.href}" data-idx="${index}" data-key="href" placeholder="URL" class="rep-field w-full bg-gray-900 text-gray-400 text-[10px] p-1.5 rounded border border-gray-600">`;
                        } else {
                            row.innerHTML = `<div class="flex gap-2"><select data-idx="${index}" data-key="icon" class="rep-field bg-gray-900 text-white text-xs p-1.5 rounded border border-gray-600 flex-1"><option value="ph-facebook-logo" ${item.icon=='ph-facebook-logo'?'selected':''}>Facebook</option><option value="ph-instagram-logo" ${item.icon=='ph-instagram-logo'?'selected':''}>Instagram</option><option value="ph-youtube-logo" ${item.icon=='ph-youtube-logo'?'selected':''}>Youtube</option><option value="ph-tiktok-logo" ${item.icon=='ph-tiktok-logo'?'selected':''}>TikTok</option><option value="ph-zalo-logo" ${item.icon=='ph-chat-circle-dots'?'selected':''}>Zalo</option><option value="ph-phone" ${item.icon=='ph-phone'?'selected':''}>Phone</option></select><button type="button" onclick="rmRepItem(${index})" class="text-gray-500 hover:text-red-500"><i class="ph ph-trash"></i></button></div><input type="text" value="${item.link}" data-idx="${index}" data-key="link" placeholder="Link..." class="rep-field w-full bg-gray-900 text-gray-400 text-[10px] p-1.5 rounded border border-gray-600">`;
                        }
                        container.appendChild(row);
                    });
                    document.querySelectorAll('.rep-field').forEach(f => f.addEventListener('input', (e) => {
                        currentData[e.target.dataset.idx][e.target.dataset.key] = e.target.value; updateRepeater();
                    }));
                }
                function updateRepeater() {
                    ...
                    if(targetEl) {
                        targetEl.setAttribute(attrName, jsonStr);
                ...
                btnAdd.onclick = () => { 
                    currentData.push(cls.includes('Menu') ? {text:'Mới', href:'#'} : {icon:'ph-link', link:'#'}); 
                    renderRepeaterUI(); updateRepeater(); 
                };
                renderRepeaterUI();
            }
        }
        ...
        inputs.forEach(input => {
            if(window.activeElement) {
                const c = window.activeElement.getAttribute('data-class');
                if(c.includes('Button')) {
                     const a = window.activeElement.querySelector('a');
                     if(input.dataset.style === 'hover_bg') input.value = a.getAttribute('data-hover-bg') || '#1d4ed8';
                     if(input.dataset.style === 'hover_color') input.value = a.getAttribute('data-hover-color') || '#ffffff';
                     if(input.dataset.style === 'background-color') a.setAttribute('data-original-bg', input.value);
                     if(input.dataset.style === 'color') a.setAttribute('data-original-color', input.value);
                }
            }
            ...
            input.addEventListener('input', function() {
                ...
                if (cls.includes('Button') || cls.includes('CategoryBtn')) target = window.activeElement.querySelector('a, button, .inner-box');
                else if (cls.includes('Search')) target = window.activeElement.querySelector('.search-box, .search-icon-only');
                else if (cls.includes('Menu') || cls.includes('Socials')) target = window.activeElement.querySelector('nav, .social-group');
                else if (cls.includes('Divider')) target = window.activeElement.querySelector('div');
                ...
                switch (type) {
                    ...
                    case 'layout': 
                        if (cls.includes('Search') || cls.includes('Cart')) {
                            // Simple reload for structure change
                            const parent = window.activeElement.parentElement;
                            const newHtml = renderElementPreview(cls.split('\\').pop(), cls); 
                        }
                        break;
                    case 'shape':
                        if(cls.includes('Socials')) {
                            ...
                            links.forEach(a => {
                                a.className = 'social-link hover:opacity-80 transition flex items-center justify-center'; // reset
                                if(val === 'circle') a.classList.add('rounded-full', 'bg-gray-100', 'p-2');
                                if(val === 'square') a.classList.add('rounded', 'bg-gray-100', 'p-2');
                            });
                        }
                        break;
                }
    ...
    function renderRepeaterHTML(el, type) {
        if (type === 'Menu') {
            ...
            try {
                const items = JSON.parse(nav.getAttribute('data-menu-config'));
                let h = ''; items.forEach(i => h += `<a href="${i.href}" class="hover:text-blue-600 transition px-1">${i.text}</a>`);
                nav.innerHTML = h;
            } catch(e){}
        }
        if (type === 'Socials') {
            const grp = el.querySelector('.social-group');
            try {
                const items = JSON.parse(grp.getAttribute('data-social-items'));
                let h = '';
                const isCircle = grp.innerHTML.includes('rounded-full');
                const isSquare = grp.innerHTML.includes('rounded') && !isCircle;
                const shapeClass = isCircle ? 'rounded-full bg-gray-100 p-2' : (isSquare ? 'rounded bg-gray-100 p-2' : '');
                items.forEach(i => h += `<a href="${i.link}" target="_blank" class="social-link hover:opacity-80 transition flex items-center justify-center ${shapeClass}"><i class="ph ${i.icon}"></i></a>`);
                grp.innerHTML = h;
            } catch(e){}
        }
    }

    // --- FIX: RENDER ELEMENT PREVIEW (CẬP NHẬT ĐỂ HIỆN ĐẸP CATEGORY & SOCIALS) ---
    // --- FIX LỖI MẤT NÚT XÓA ---
    // Thay thế toàn bộ hàm renderElementPreview cũ bằng hàm này
    function renderElementPreview(name, className) {
        let content = '';
        let extraClass = ''; // Dùng để chỉnh width nếu cần (ví dụ search box full width)

        // 1. Tạo nội dung HTML cho từng loại Element
        if (className.includes('Button')) {
            ...
            content = `<form class="search-box relative flex items-center h-9 w-full bg-gray-100 rounded-full pointer-events-none" style="min-width:200px"><input type="text" placeholder="Tìm kiếm..." class="w-full h-full px-4 border-0 bg-transparent text-xs"><i class="ph ph-magnifying-glass absolute right-3 opacity-50"></i></form>`;
            extraClass = 'w-full';
        }
        else if (className.includes('Menu')) {
            content = `<nav class="flex items-center gap-6 text-sm font-medium whitespace-nowrap overflow-hidden text-inherit pointer-events-none menu-nav"><span>Điện thoại</span><span>Laptop</span></nav>`;
            extraClass = 'w-full';
        }
            ...
            content = `<div class="flex flex-col items-center pointer-events-none px-2 text-inherit"><i class="ph ph-shopping-cart text-2xl"></i><span class="text-[10px]">Giỏ hàng</span></div>`;
        }
            ...
            // Fallback cho các element chưa định nghĩa
            content = `<span class="font-bold text-sm pointer-events-none">${name}</span>`;
        }

        // 2. Trả về cấu trúc CHUẨN bao gồm nút DELETE (btn-delete)
        return `
            <div class="builder-element relative group cursor-pointer hover:bg-blue-50/50 p-1 rounded transition border border-transparent hover:border-blue-300 flex items-center justify-center ${extraClass}" data-class="${className}">
                ...
                <!-- NÚT XÓA (Đây là cái bị thiếu lúc nãy) -->
