document.addEventListener('DOMContentLoaded', () => {
    ...
    style.innerHTML = `
        .hb-inner-content { display: flex; align-items: center; width: 100%; height: 100%; margin: 0 auto; min-height: 40px; }
        .drop-zone { 
            height: 100%; min-height: 40px; 
            display: flex; align-items: center; 
            padding: 0 10px; transition: all 0.2s; border: 1px dashed transparent; 
            /* Min width nhỏ để dễ thả khi rỗng, nhưng không ảnh hưởng layout khi có content */
            min-width: 50px; 
        }
        .empty-placeholder { color: #9ca3af; font-size: 11px; font-weight: 500; display: flex; align-items: center; pointer-events: none; white-space: nowrap; }
        .drop-zone:empty, .drop-zone:has(.empty-placeholder) { border-color: rgba(0,0,0,0.05); }
        .builder-row:hover .drop-zone { border-color: #d1d5db; }
        .drop-zone.drag-over { background: rgba(59, 130, 246, 0.1) !important; border: 2px dashed #3b82f6 !important; }
    ...
    // --- 4. LAYOUT (FIX: SMART FLEX - SÁT NỘI DUNG) ---
    function ensureInnerContent(row) {
        ...
        // Cập nhật Flex cho từng zone
        const zones = inner.querySelectorAll('.drop-zone');
        zones.forEach(zone => {
            const zName = zone.getAttribute('data-zone') || '';
            
            // Xóa class/style cũ
            zone.classList.remove('flex-1'); 
            zone.style.flex = ''; 
            zone.style.width = '';
            
            if (zName.includes('center')) {
                // Center: Chiếm hết khoảng trống còn lại (Flex Grow = 1)
                zone.style.flex = '1 1 auto'; 
                zone.style.justifyContent = 'center';
            } else {
                // Left/Right: Chỉ rộng bằng nội dung (Flex Grow = 0)
                // shrink = 0 để không bị bóp méo
                zone.style.flex = '0 0 auto'; 
                
                if (zName.includes('left')) zone.style.justifyContent = 'flex-start';
                if (zName.includes('right')) zone.style.justifyContent = 'flex-end';
            }
    ...
    function handleDragStart(e) { e.dataTransfer.setData('text/plain', this.dataset.class); e.dataTransfer.setData('name', this.innerText.trim()); }
    function handleDragOver(e) { e.preventDefault(); this.classList.add('drag-over'); }
    function handleDragLeave(e) { this.classList.remove('drag-over'); }
    function handleDrop(e) {
        e.preventDefault(); this.classList.remove('drag-over');
        const cls = e.dataTransfer.getData('text/plain'); const name = e.dataTransfer.getData('name');
        if (cls) {
            const ph = this.querySelector('.empty-placeholder'); if (ph) ph.remove();
    ...
    function attachElementEvents(el, cls) {
        ...
        if(btn) btn.addEventListener('click', (e) => { e.stopPropagation(); el.remove(); propertyPanel.innerHTML = '<div class="p-5 text-center text-gray-500 text-xs">Deleted</div>'; });
        ...
        data.forEach(rowData => {
            ...
            if (rowData.columns) {
                for (const [zoneKey, items] of Object.entries(rowData.columns)) {
                    ...
                    if (items.length > 0) { const ph = zone.querySelector('.empty-placeholder'); if(ph) ph.remove(); }
        ...
        inputs.forEach(input => {
            ...
            input.addEventListener('input', function() {
                ...
                switch (type) {
                    ...
                    case 'width': if (cls.includes('Search')) target.style.width = val + '%'; else target.style.width = val.includes('%') ? val : val + 'px'; break;
    ...
    function getStyleTarget(root, cls) { if(!cls) return root; if(cls.includes('Button')||cls.includes('CategoryBtn')) return root.querySelector('.inner-box'); if(cls.includes('Search')) return root.querySelector('.search-box, .search-icon-only'); if(cls.includes('Menu')||cls.includes('Socials')) return root.querySelector('nav, .social-group'); if(cls.includes('Divider')) return root.querySelector('div'); if(cls.includes('Text')) return root.querySelector('.text-content'); if(cls.includes('Logo')) return root.querySelector('img'); return root; }
    function renderElementPreview(name, className, contentData = {}) {
        ...
        else if (className.includes('Search')) { const h = c.height || 36; if (c.layout === 'icon') content = `<div class="search-icon-only cursor-pointer hover:opacity-70" style="color: inherit; font-size: 20px;"><i class="ph ph-magnifying-glass"></i></div>`; else { content = `<form class="search-box relative flex items-center w-full bg-gray-100 rounded-full pointer-events-none" style="min-width:200px; height: ${h}px"><input type="text" placeholder="Tìm kiếm..." class="w-full h-full px-4 border-0 bg-transparent text-xs"><i class="ph ph-magnifying-glass absolute right-3 opacity-50"></i></form>`; extraClass = 'w-full'; } }
    ...
    viewBtns.forEach(btn => { btn.addEventListener('click', () => { canvasFrame.style.maxWidth = (btn.dataset.mode === 'mobile') ? '375px' : (btn.dataset.mode === 'tablet' ? '768px' : '100%'); }); });
});